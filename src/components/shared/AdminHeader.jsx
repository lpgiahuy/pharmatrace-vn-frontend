...
import { MenuFoldOutlined, MenuUnfoldOutlined, BellOutlined, UserOutlined, MenuOutlined } from '@ant-design/icons'
...
export const AdminHeader = ({ collapsed, onToggle, portal = 'Admin', isMobile }) => {
  ...
  return (
    <Header
      style={{ background: '#fff', padding: isMobile ? '0 16px' : '0 24px', borderBottom: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', height: 64, position: 'sticky', top: 0, zIndex: 99 }}
    >
      <button onClick={onToggle} className="p-2 rounded-lg hover:bg-slate-100 transition-colors mr-2 sm:mr-4 text-slate-600">
        {isMobile ? <MenuOutlined style={{ fontSize: 18 }} /> : (collapsed ? <MenuUnfoldOutlined style={{ fontSize: 18 }} /> : <MenuFoldOutlined style={{ fontSize: 18 }} />)}
      </button>
